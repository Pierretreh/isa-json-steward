...
class ProfileLoader:
    ...
    def __init__(self, profile_path: Optional[str] = None) -> None:
        ...
        if profile_path is not None:
            self._config_dir = Path(profile_path).resolve()
        else:
        ...
        logger.debug("ProfileLoader initialised with config_dir=%s", self._config_dir)
    ...
    def get_subdirectory_assay_patterns(self) -> Dict[str, str]:
        ...
        return dict(
            self._get_section("experiment_patterns").get("subdirectory_assay_patterns", {})
        )
